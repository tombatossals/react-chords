...
const Chord = ({ chord, tunning, version }) => {
  ...
  const barres = position.barres.map(barre => firstFret > 1 ? barre - firstFret + 1 : barre)
  ...
  return <svg
    ...
    <g
      ...
      {barres.map((barre, index) => <Barre key={index} barre={barre} frets={frets} fingers={position.fingers} />)}
